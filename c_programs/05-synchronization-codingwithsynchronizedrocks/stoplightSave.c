...
int parse_args(int argc, char **argv)
{
    if (argc != 3)
    {
        fprintf(stderr, "Usage: %s <time> <cars>\n", argv[0]);
        return 1;
    }
...
int simulate_intersection(car_t *this_car)
{
    ...
    nextIn[this_car->appr_dir] = *this_car; // adds to an array of whose in intersection ( may delete )

    test(this_car, &can_proceed);

    if (can_proceed == 1)
    {
        // simulate going through the intersection, this is our drop out point which should post() eventually leaving and reset lane_states
        /* move through intersection with function that looks @ #1's to get size of movemment
         * Then compare current direction */
        traverse_intersection(this_car);
        leave_intersect(this_car);
        // call test on the other cars in array
        for (i = 0; i < DIRMAX; i++)
        {
            if (i != this_car->appr_dir)
            {
                test(&this_car[i], &can_proceed);
            }
        }
    }

    // want barrier to gather cars in lane  
...
{
    /**
     * In test we check lane availability, decrement if available
     * need to keep 1 thread checking and decrementing if available
     * split up checking and decrementing?
     * Goal:
     *  Once we test and it fails, need thread to wait on working thread to finish
     *  increment a counter that indicates a thread is blocked
     */
    int i;
...
int initalize_semaphores()
{
    ...
    sem_ret = semaphore_create(&testS, 1);
    if (sem_ret != 0)
    {
        fprintf(stderr, "Error test() mutex\n");
        sem_ret = 0;
